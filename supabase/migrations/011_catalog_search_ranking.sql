...
language sql stable security invoker set search_path = '' as $$
  ...
  order by case when p_sort='next-deadline' then (select min(point->>'date') from jsonb_array_elements(coalesce(m.item->'milestones','[]')) point where point->>'kind'='deadline' and coalesce((point->>'superseded')::boolean,false)=false and (point->>'date')::date>=current_date) end asc nulls last,
