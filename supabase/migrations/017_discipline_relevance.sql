...
create or replace function public.search_opportunities(
  ...
  p_modes text[] default '{}', p_funding text[] default '{}', p_free_submission boolean default false,
...
language sql stable security invoker set search_path = '' as $$
  with query_terms as (
    ...
    select s.*,
      case when cardinality(p_disciplines)=0 then 0::real else coalesce((
        select max(coalesce(
          (select value::real from jsonb_each_text(coalesce(s.item->'disciplineAffinity','{}')) affinity
            where lower(affinity.key)=lower(selected) limit 1),
          case when lower(s.item->>'discipline')=lower(selected) then 100::real else 60::real end
        )) from unnest(p_disciplines) selected
        where exists(select 1 from jsonb_array_elements_text(coalesce(s.item->'disciplines',jsonb_build_array(s.item->>'discipline'))) d where lower(d)=lower(selected))
      ),0::real) end discipline_affinity
    from ranked s where
      ...
      and (cardinality(p_funding)=0 or (
        ('paid'=any(p_funding) and lower(coalesce(s.item->'costs'->>'compensation','')) ~ '(paid|stipend|salary|award|[$][0-9])' and lower(coalesce(s.item->'costs'->>'compensation','')) !~ '(^|\m)no (wage|stipend|compensation)')
        or ('aid'=any(p_funding) and lower(coalesce(s.item->'costs'->>'aid','')) ~ '(aid|waiver|scholarship|assistance|reimbursement|cover)')
        or ('no-program-fee'=any(p_funding) and lower(coalesce(s.item->'costs'->>'program','')) ~ '(^|\m)(free|no (attendance|participation|program|tuition)? ?(cost|fee))')
      ))
      and (not p_free_submission or lower(concat_ws(' ',s.item->'costs'->>'application',s.item->'costs'->>'submission')) ~ '(^|\m)(free|no (application |submission )?fee)(\M|$)')
  ...
  order by
    ...
    case when p_sort='relevance' and cardinality(p_disciplines)>0 then m.discipline_affinity end desc nulls last,
